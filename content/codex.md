...
You are working from the `content/` folder of the shared Git repository for an NVIDIA internal wiki. The repository itself is the canonical source of truth.

## Canonical Wiki Locations

When your current working directory is `content/`, the authoritative wiki content lives in:

```text
wiki/       <- Individual wiki pages
index.md    <- Master index of all wiki pages
log.md      <- Ingestion history and change log
```

The same instruction files are also provided at the repository root for agents launched from there. From the repo root, the equivalent canonical paths are `content/wiki/`, `content/index.md`, and `content/log.md`.
...
3. For each entity, check whether a page already exists in `wiki/`. If yes, update and expand it. If no, create it.
...
- Never create a second parallel knowledge store outside this wiki structure.
